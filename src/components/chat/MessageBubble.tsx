...
const formatText = (text: string, highlight?: string): React.ReactNode[] => {
  ...
  const processInline = (line: string): React.ReactNode[] => {
    ...
    while ((m = regex.exec(line)) !== null) {
      ...
      else if (m[5]) inlineParts.push(<code key={key++} className="bg-black/30 px-1.5 py-0.5 rounded text-[0.88em] font-mono">{m[5]}</code>);
      else if (m[6]) inlineParts.push(<s key={key++}>{highlightText(m[6], highlight, key)}</s>);
      else if (m[7]) inlineParts.push(<SpoilerText key={key++} text={m[7]} />);
      else if (m[8]) inlineParts.push(<span key={key++} className="text-primary cursor-pointer hover:underline">#{m[8]}</span>);
      else if (m[9]) inlineParts.push(<span key={key++} className="text-primary font-semibold cursor-pointer hover:underline">@{m[9]}</span>);
      else if (m[10]) inlineParts.push(<a key={key++} href={m[10]} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all">{m[10]}</a>);
  ...
  segments.forEach((seg, si) => {
    if (seg.type === 'code') {
      parts.push(
        <pre key={`code-${si}`} className="bg-black/40 rounded-lg p-3 my-1.5 text-xs font-mono overflow-x-auto">
      ...
      textLines.forEach((line, li) => {
        ...
        if (line.startsWith('> ')) {
          parts.push(
            <div key={`bq-${si}-${li}`} className="border-l-2 border-primary/50 pl-2 py-0.5 my-1 text-muted-foreground italic bg-primary/[0.05] rounded-r">
...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  if (message.type === 'service') {
    return (
      <div id={`msg-${message.id}`} className="flex justify-center my-2 animate-[msgIn_0.2s_ease-out]">
  ...
  return (
    <div
      id={`msg-${message.id}`}
      className={`flex ${isOwn && !isChannel ? 'justify-end' : 'justify-start'} mb-1 relative group animate-[msgIn_0.2s_ease-out] ${isCurrentSearchMatch ? 'bg-dex-warning/10 rounded-lg' : isSearchMatch ? 'bg-primary/[0.05] rounded-lg' : ''}`}
    ...
    >
      ...
      <div
        className={`relative px-3 py-2 ${isChannel ? 'w-full max-w-full rounded-xl bg-dex-bubble-other' : isOwn ? 'max-w-[480px] rounded-[18px_18px_4px_18px] bg-gradient-to-br from-primary to-[hsl(252,60%,48%)]' : 'max-w-[480px] rounded-[18px_18px_18px_4px] bg-dex-bubble-other'} ${message.pinned ? 'border border-primary/30' : ''} ${isSelected ? 'ring-2 ring-primary/50' : ''}`}
      >
        {/* Scheduled indicator */}
        {message.scheduled && (
          <div className="flex items-center gap-1 text-[10px] text-dex-warning mb-1">🕐 Scheduled: {message.scheduledTime}</div>
        )}

        {message.pinned && <div className="flex items-center gap-1 text-[10px] text-primary mb-1">📌 Pinned</div>}
        ...
        {message.type === 'poll' && message.pollData && (
          <PollMessage pollData={message.pollData} onVote={(idx) => onVotePoll(message.id, idx)} />
        )}

        {/* Text content */}
        {message.type === 'message' && (
          <div className="text-sm leading-relaxed whitespace-pre-wrap break-words">
            {formatText(displayText, searchHighlight || undefined)}
          </div>
        )}

        {/* Translated indicator */}
        {message.translated && (
          <div className="text-[10px] text-primary/60 mt-1">🌐 Translated</div>
        )}
        ...
        {/* Silent indicator */}
        {message.silentSend && (
          <div className="text-[10px] mt-0.5 opacity-50">🔕 Sent silently</div>
        )}
